...
3. **Logos MCP Server (Model Context Protocol):** 
   Bereitstellung der Logos Table als offiziellen MCP-Server. Statt einer generischen API wird die App zu einer Headless Wissens-Engine für LLMs (wie Claude, Cursor oder mich). Dies ermöglicht es KI-Agenten nativ über bereitgestellte "Tools":
   - Die philosophische Datenbank (Atome und Probleme) zu durchsuchen und logische Querbezüge abzufragen.
   - Den Lean 4- und Python-Evaluator der App fernzusteuern, um automatisch neue formale Beweise zu verifizieren.
   - Selbstständig neue LeetCode-Probleme zu analysieren und direkt als standardkonforme JSON-Drafts in das Periodensystem einzupflegen.

### Definition of Done

Ein User kann das Periodensystem als 3D-Netz erleben, Lean-Beweise interaktiv durchklicken und die App als MCP-Server in seiner LLM-Umgebung aktivieren, sodass die KI das philosophische System eigenständig nutzen und erweitern kann.
